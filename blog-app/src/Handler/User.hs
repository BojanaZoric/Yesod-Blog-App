...
instance ToJSON Register where
 toJSON (Register registerUsername registerEmail registerPassword) =
...
postUserRegisterR = do
    ...
    let user = User (registerUsername registerData) ( registerEmail registerData) hashedPwd "admin"
    insertedUser <- runDB $ insert user
    returnJson insertedUser
